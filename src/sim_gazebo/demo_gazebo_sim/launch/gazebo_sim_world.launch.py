...
def generate_launch_description():
    ...
    robot_desc_launch = IncludeLaunchDescription(
       launch_file_path=os.path.join(
        get_package_share_directory("mycar_description"),
            "launch",
    ...
    dridge = Node(
        ...
        arguments=[
            ...
            "/model/mycar/odometry@nav_msgs/msg/Odometry[gz.msgs.Odometry", # 里程计
            "/model/mycar/tf@tf2_msgs/msg/TFMessage[gz.msgs.Pose_V", # 坐标变换
            "/world/empty/model/mycar/joint_state@sensor_msgs/msg/JointState[gz.msgs.model", # 关节状态
            "/clock@rosgraph_msgs/msg/Clock[gz.msgs.Clock", # 时钟
            # 单线雷达
            "//scan@sensor_msgs/msg/LaserScan[gz.masg.LaserScan",
            # 多线雷达
            "/scan/points@sensor_msgs/msg/PointCloud2[gz.msgs.PointCloudPacked",
            "/image_raw@sensor_msgs/msg/Image[gz.msgs.Image", # 图片消息
            "/camera_info@sensor_msgs/msg/CameraInfo[gz.msgs.CameraInfo", #相机参数
            "/depth_camera@sensor_msgs/msg/Image[gz.msgs.Image",  # 深度相机灰度图
            "/depth_camera/points@sensor_msgs/msg/PointCloud2[gz.msgs.PointCloudPacked" #深度相机的点云数据
        ],
        remappings=[
            ("/model/mycar/tf","/tf"),
    ...
    # bug:遇到小车不在原点,在其他位置时,执行以下操作
    # 在终端命令行中输入: ps aux | grep -i ign 查看进程中是否存在
    # ign_gazebo_server 若存在要去关闭这个服务
    # 如何关闭: 在终端中输入:kill -9 第二个参数一般是:97322 进行关闭
    # 继续在终端中输入: ps aux | grep -i ign 查看是否关闭成功



    # 遇到小车没有显示图图片时执行以下操作
    # ros2 run tf2_ros static_transform_publisher --frame-id camera --child-frame-id mycar/base_footprint/depth_camera
