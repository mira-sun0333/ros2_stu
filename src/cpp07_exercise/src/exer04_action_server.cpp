...
/*
    ...
    流程:
        ...
        3.自定义节点类
            ...
            3-2创建一个速度指令发布方,控制乌龟的运动
            3-3创建一个动作服务端
            3-4解析客户端提交的数据
            3-5处理客户端的取消请求操作
            3-6实现主逻辑(耗时操作),启动子线程
            3-7子线程中,发布速度指令,产生连续反馈,并响应最终结果
...
public:
    Exer04ActionServer():Node("exer04_action_server_node_cpp"),x(0.0),y(0.0){
        ...
        //3-1创建原生乌龟位姿订阅方,获取当前乌龟坐标
        sub_ = this->create_subscription<turtlesim::msg::Pose>("turtle/pose",10,std::bind(&Exer04ActionServer::pose_cb,this,_1));
        //3-2创建一个速度指令发布方,控制乌龟的运动
...
private:
    ...
    rclcpp_action::GoalResponse handle_goal(const rclcpp_action::GoalUUID & uuid, std::shared_ptr<const typename Nav::Goal>goal){
        (void)uuid;
        //(void)goal;
        //取出目标中的x y 坐标,分别判断是否超出了[0,11.08]取值,如果超出,则认为非法,否则合法
